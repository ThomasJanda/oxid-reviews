...
<div id="review">
    ...
    [{assign var=bHasReviews value=false}]
    [{assign var=aList value=$oView->getReviewsAll()}]
    [{foreach key=langid from=$aList item=aItem}]
        [{assign var=bHasReviews value=true}]
        [{assign var=oReviews value=$aItem.reviews}]
        
        [{capture append="rs_reviews_buttons"}]
            <button type="button" data-langid="[{$langid}]" class="btn [{if $aItem.selected}] btn-primary [{/if}]">
            [{oxmultilang ident="rs_reviews__display_review_in_lang_"|cat:$langid}] ([{$oReviews->count()}] [{oxmultilang ident="rs_reviews__reviews"}])
            </button>
        [{/capture}]
        
        [{capture append="rs_reviews_reviews"}]
            <div id="rs_reviews_[{$langid}]">
                <div class="reviews-landscape">
                    [{foreach from=$oReviews item=review name=ReviewsCounter}]
    ...
    [{if $bHasReviews}]
        [{oxstyle include=$oViewConf->getModuleUrl("rs-reviews", "out/src/css/css.css")}]
        [{oxscript include=$oViewConf->getModuleUrl("rs-reviews", "out/src/js/js.js")}]
        <div class="rs_reviews_button_line">
            [{foreach from=$rs_reviews_buttons item="_block"}]
                [{$_block}]
            [{/foreach}]
        </div>
        <div class="rs_reviews_reviews_list">
            [{foreach from=$rs_reviews_reviews item="_block"}]
                [{$_block}]
            [{/foreach}]
        </div>
    [{else}]
